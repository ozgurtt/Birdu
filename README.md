...
sed -i '1s/^/export ANDROID_HOME="\/home\/james\/Android\/Sdk\/"\n/' ~/.bashrc
 #need to export ANDROID_HOME to bash's PATH. Use 'sed' to insert a ANDROID_HOME's path into the bashrc profile, permanently adding this path variable. You can check your bashrc with 'nano ~/.bashrc', and the ANDROID_HOME should be the first line
